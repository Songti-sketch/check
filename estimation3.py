...
class Estimator:
    ...
    def objective_beta(beta, cust_idx, sell_idx, omega, p_m, factor, dist, num_total_cells, fixed_assortments, penalty=0):
        try:
            ...
            log_p_purchase = np.log(p[cust_idx - 1, sell_idx - 1] + 1e-15)
            log_p_no_purchase = np.log(p[:, -1] + 1e-15)
            reg = penalty * np.sum(beta**2)
            return -(np.sum(log_p_purchase) + factor * np.sum(omega * log_p_no_purchase) - reg)
        except:
            return np.inf
    ...
    @staticmethod
    def EM(dist_per_grid, df, dist, num_total_cells, fixed_assortments, max_iter=200, tol=1e-4, penalty=0):
        N = len(df)
        ...
        for iteration in range(max_iter):
            ...
            c = np.array([(df['cust_index'] == i + 1).sum() + factor * omega[i] * p_m[i, -1] 
                          for i in range(num_total_cells)])
            omega_new = c / (c.sum() + 1e-10)
            res = minimize(
    ...
    def calculate_wmape(extra_assortment, p_m, omega, beta, lambda_hat, dist, eval_df, dist_per_grid, timeperiod):
        ...
        relative_errors = np.where(o_ij_actual > 0, (np.abs(o_ij_actual - o_ij_hat) / o_ij_actual) * 100, 0)
        # pdb.set_trace()
        # relative_errors = (np.abs(o_ij_actual - o_ij_hat) / o_ij_actual) * 100 
        # avoid error report:
